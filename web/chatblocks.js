  ...
  theCode = `<html>
	...
	<body>
		...
		try {
			...
			ComfyJS.Init( "${
        window.localStorage.getItem("channel") || ComfyTwitch.User
      }", "oauth:${ComfyTwitch.Token}" );
...
document.getElementById("run-code").addEventListener("click", (ev) => {
  if (isCodeRunning) {
    ...
    document.getElementById(
      "run-code"
    ).innerHTML = `<i class="fa fa-play"></i> Run Blocks`;
    ...
    document.getElementById(
      "run-code"
    ).innerHTML = `<i class="fa fa-stop"></i> Stop Blocks`;
    let sandbox = document.getElementById("run-sandbox");
    sandbox.innerHTML = "";
...
document.getElementById("blocks-file-input").addEventListener(
  "change",
  async function () {
...
Blockly.JavaScript["text_contains"] = function (block) {
  var value_src = Blockly.JavaScript.valueToCode(
    block,
    "SOURCE",
    Blockly.JavaScript.ORDER_ATOMIC
  );
  var value_contains = Blockly.JavaScript.valueToCode(
    block,
    "CONTAINS",
    Blockly.JavaScript.ORDER_ATOMIC
  );
...
Blockly.JavaScript["text_countAmount"] = function (block) {
  var value_occuranceof = Blockly.JavaScript.valueToCode(
    block,
    "OCCURANCEOF",
    Blockly.JavaScript.ORDER_ATOMIC
  );
...
Blockly.JavaScript["utility_save_data"] = function (block) {
  var key_value = Blockly.JavaScript.valueToCode(
    block,
    "KEY",
    Blockly.JavaScript.ORDER_ATOMIC
  );
...
Blockly.JavaScript["math_eval"] = function (block) {
  var value_exp = Blockly.JavaScript.valueToCode(
    block,
    "EXPRESSION",
    Blockly.JavaScript.ORDER_ATOMIC
  );
...
Blockly.Blocks["json_create_with"] = {
  ...
  updateShape_: function () {
    if (this.itemCount_ && this.getInput("EMPTY")) {
      ...
      this.appendDummyInput("EMPTY").appendField("create empty json");
    }
    // Add new inputs.
    for (var i = 0; i < this.itemCount_; i++) {
      if (!this.getInput("KEY" + i)) {
        let input = this.appendValueInput("KEY" + i).setAlign(
          Blockly.ALIGN_RIGHT
        );
        let input2 = this.appendValueInput("VALUE" + i).setAlign(
          Blockly.ALIGN_RIGHT
        );
        if (i == 0) {
          input.appendField("create json with");
        }
...
Blockly.JavaScript["json_create_with"] = function (block) {
  ...
  for (var i = 0; i < block.itemCount_; i++) {
    let key = Blockly.JavaScript.valueToCode(
      block,
      "KEY" + i,
